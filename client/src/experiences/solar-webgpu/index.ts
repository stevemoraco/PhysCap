...
const providedCanvas = globalScope.__PHYSICAL_CAPITAL_SOLAR_CANVAS__;
const canvasOrNull =
  providedCanvas ||
...
if (!canvasOrNull) {
  throw new Error('Physical.Capital solar WebGPU canvas was not provided.');
}

const canvas: HTMLCanvasElement = canvasOrNull;

if (providedCanvas) {
  delete globalScope.__PHYSICAL_CAPITAL_SOLAR_CANVAS__;
}
...
if (typeof globalScope.__PHYSICAL_CAPITAL_SOLAR_QUALITY__ === 'number') {
  qualityScale = Math.max(
    0.3,
    Math.min(1, globalScope.__PHYSICAL_CAPITAL_SOLAR_QUALITY__),
  );
...
function handleResize() {
  ...
  const displayWidth = Math.max(
    1,
    Math.floor((canvas.clientWidth || canvas.width) * pixelRatio),
  );
  const displayHeight = Math.max(
    1,
    Math.floor((canvas.clientHeight || canvas.height) * pixelRatio),
  );
...
export function onCleanup() {
  ...
  canvas.removeEventListener('mousedown', dismissAttribution);
  canvas.removeEventListener('touchstart', dismissAttribution);
  canvas.removeEventListener('wheel', dismissAttribution);
